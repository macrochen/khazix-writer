...
8. **数字一律用阿拉伯数字**：所有涉及数字的地方必须使用阿拉伯数字（0-9），禁止使用汉字数字（一二三四五六七八九十百千万）。例如写"45天"不写"四十五天"，写"50%"不写"百分之五十"，写"3个"不写"三个"。例外情况：(1)成语、固定表达中的数字（如"三言两语"、"七上八下"）；(2)纯粹修辞的模糊数字（如"好几百个"、"几千块"）；(3)序数词连接用法（"第一，xxx"、"第二，xxx"）保留汉字。
...
**L1-5 数字格式扫描**
全文搜索汉字数字，出现则必须替换为阿拉伯数字：
- "百分之五十" → "50%"
- "四十五天" → "45天"
- "三个" → "3个"
- "十二" → "12"
- 以此类推。例外情况：(1)成语（"三言两语"）；(2)模糊修辞（"好几百个"）；(3)序数词连接用法（"第一，xxx"、"第二，xxx"）保留汉字。
- 特别注意"百分之X"结构，一律改为"X%"。
